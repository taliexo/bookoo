...
class TestDetectChanneling(unittest.TestCase):
    def setUp(self):
        ...
        # Using default config for most tests. Specific configs can be used if needed.
    ...
    def test_no_profile_or_insufficient_data(self: "TestDetectChanneling") -> None:
        ...
        short_profile: FlowProfile = [FlowDataPoint(1.0, 0.1), FlowDataPoint(2.0, 0.2)]
        self.assertEqual(self.analyzer.detect_channeling(empty_profile), "Undetermined")
        self.assertEqual(
            self.analyzer.detect_channeling(short_profile),
            "Undetermined (not enough data after initial phase)",
        )
        profile_all_initial: FlowProfile = [
            FlowDataPoint(elapsed_time=float(i), flow_rate=0.5 + i * 0.1)
            for i in range(7)
        ]  # Assumes initial_ignore_seconds = 7.0
        self.assertEqual(
            self.analyzer.detect_channeling(profile_all_initial),
            "Undetermined (not enough data after initial phase)",
        )

    def test_clear_non_channeling_shot(self: "TestDetectChanneling") -> None:
        stable_profile: FlowProfile = [
            FlowDataPoint(elapsed_time=float(i), flow_rate=0.2 * i) for i in range(7)
        ] + [FlowDataPoint(elapsed_time=float(i), flow_rate=1.5) for i in range(7, 27)]
        self.assertEqual(self.analyzer.detect_channeling(stable_profile), "None")

    def test_high_coefficient_of_variation(self: "TestDetectChanneling") -> None:
        unstable_profile: FlowProfile = (
            [FlowDataPoint(elapsed_time=float(i), flow_rate=0.2 * i) for i in range(7)]
            + [
                FlowDataPoint(7.0, 1.0),
                FlowDataPoint(8.0, 2.5),
                FlowDataPoint(9.0, 0.8),
                FlowDataPoint(10.0, 2.2),
                FlowDataPoint(11.0, 0.5),
                FlowDataPoint(12.0, 3.0),
                FlowDataPoint(13.0, 0.7),
                FlowDataPoint(14.0, 2.8),
                FlowDataPoint(15.0, 1.0),
                FlowDataPoint(16.0, 2.5),
            ]
            + [
                FlowDataPoint(elapsed_time=float(i), flow_rate=1.5 + (0.5 * (i % 2)))
                for i in range(17, 27)
            ]
        )
        self.assertEqual(
            self.analyzer.detect_channeling(unstable_profile),
    ...
    def test_significant_spike(self: "TestDetectChanneling") -> None:
        spike_profile: FlowProfile = (
            [FlowDataPoint(elapsed_time=float(i), flow_rate=0.2 * i) for i in range(7)]
            + [
                FlowDataPoint(elapsed_time=float(i), flow_rate=1.5)
                for i in range(7, 15)
            ]
            + [FlowDataPoint(15.0, 4.0)]
            + [
                FlowDataPoint(elapsed_time=float(i), flow_rate=1.6)
                for i in range(16, 27)
            ]
        )
        self.assertEqual(
            self.analyzer.detect_channeling(spike_profile),
            "Suspected Channeling (Spike)",
        )
    ...
    def test_high_cv_and_spike(self: "TestDetectChanneling") -> None:
        unstable_spike_profile: FlowProfile = (
            ...
            + [
                FlowDataPoint(7.0, 1.0),
                FlowDataPoint(8.0, 2.0),
                FlowDataPoint(9.0, 1.2),
                FlowDataPoint(10.0, 4.5),  # Spike
                FlowDataPoint(11.0, 0.9),
                FlowDataPoint(12.0, 2.2),
                FlowDataPoint(13.0, 1.1),
                FlowDataPoint(14.0, 1.9),
                FlowDataPoint(15.0, 1.3),
                FlowDataPoint(16.0, 1.7),
            ]
            + [
                FlowDataPoint(elapsed_time=float(i), flow_rate=1.5 + (0.6 * (i % 2)))
                for i in range(17, 27)
            ]  # Variation
        )
        self.assertEqual(
            self.analyzer.detect_channeling(unstable_spike_profile),
            "Moderate Channeling (High Variation & Spike)",
        )

    def test_low_flow_shot(self: "TestDetectChanneling") -> None:
        low_flow_profile: FlowProfile = [
            FlowDataPoint(elapsed_time=float(i), flow_rate=0.1 * i) for i in range(7)
        ] + [FlowDataPoint(elapsed_time=float(i), flow_rate=0.2) for i in range(7, 27)]
        self.assertEqual(self.analyzer.detect_channeling(low_flow_profile), "None")

    def test_no_significant_flow_after_ignore(self: "TestDetectChanneling") -> None:
        no_sig_flow_profile: FlowProfile = (
            [FlowDataPoint(elapsed_time=float(i), flow_rate=0.5 * i) for i in range(7)]
            + [
                FlowDataPoint(elapsed_time=float(i), flow_rate=0.05)
                for i in range(7, 27)
            ]  # flow is <= 0.1, so 'flows' list will be empty
        )
        self.assertEqual(
            self.analyzer.detect_channeling(no_sig_flow_profile),
...
class TestIdentifyPreInfusion(unittest.TestCase):
    ...
    def test_no_profile(self: "TestIdentifyPreInfusion") -> None:
        self.assertEqual(
            self.analyzer.identify_pre_infusion([], self.empty_stp), (False, None)
        )

    def test_too_short_profile(self: "TestIdentifyPreInfusion") -> None:
        profile: FlowProfile = [FlowDataPoint(0.5, 0.1)]
        self.assertEqual(
            self.analyzer.identify_pre_infusion(profile, self.empty_stp), (False, None)
        )

    def test_in_pi_phase_early_shot(self: "TestIdentifyPreInfusion") -> None:
        profile: FlowProfile = [
            FlowDataPoint(1.0, 0.1),
            FlowDataPoint(2.0, 0.15),
            FlowDataPoint(3.0, 0.2),
        ]  # min_shot_time_for_pi_check = 1.0, current_time = 3.0
        is_pi, duration = self.analyzer.identify_pre_infusion(profile, self.empty_stp)
    ...
    def test_in_pi_phase_sustained_low_flow(self: "TestIdentifyPreInfusion") -> None:
        profile: FlowProfile = [
            FlowDataPoint(elapsed_time=float(i * 0.5) + 1.0, flow_rate=0.15)
            for i in range(6)
    ...
    def test_exited_pi_phase(self: "TestIdentifyPreInfusion") -> None:
        ...
        profile: FlowProfile = [
            FlowDataPoint(1.0, 0.1),
            FlowDataPoint(2.0, 0.2),
            FlowDataPoint(3.0, 0.25),
            FlowDataPoint(4.0, 1.0),
            FlowDataPoint(5.0, 1.5),
        ]
    ...
    def test_shot_too_long_for_pi(self: "TestIdentifyPreInfusion") -> None:
        profile: FlowProfile = [
            FlowDataPoint(elapsed_time=float(i + 1), flow_rate=0.2) for i in range(16)
    ...
    def test_no_clear_pi_high_flow_early(self: "TestIdentifyPreInfusion") -> None:
        profile: FlowProfile = [
            FlowDataPoint(1.0, 1.0),
            FlowDataPoint(2.0, 1.5),
            FlowDataPoint(3.0, 1.2),
        ]
        is_pi, duration = self.analyzer.identify_pre_infusion(profile, self.empty_stp)
        self.assertFalse(is_pi)
        self.assertIsNone(duration)
    ...
    def test_pi_duration_too_short_is_nulled(self: "TestIdentifyPreInfusion") -> None:
        ...
        profile_at_1_5: FlowProfile = [FlowDataPoint(1.0, 0.1), FlowDataPoint(1.5, 0.2)]
        is_pi_at_1_5, duration_at_1_5 = self.analyzer.identify_pre_infusion(
            profile_at_1_5, self.empty_stp
        )
        self.assertFalse(
            is_pi_at_1_5
        )  # If duration is too short, it's not considered PI
        self.assertIsNone(
            duration_at_1_5
        )  # Duration is nulled because it's < pi_min_duration_meaningful
...
class TestCalculateExtractionUniformity(unittest.TestCase):
    ...
    def test_no_profile(self: "TestCalculateExtractionUniformity") -> None:
        self.assertEqual(self.analyzer.calculate_extraction_uniformity([]), 0.0)

    def test_insufficient_data_after_ignore(
        self: "TestCalculateExtractionUniformity",
    ) -> None:
        profile: FlowProfile = [
            FlowDataPoint(elapsed_time=float(i), flow_rate=0.5) for i in range(8)
        ]  # initial_ignore_seconds = 7.0, so only 1 data point [7.0, 0.5]
        self.assertEqual(self.analyzer.calculate_extraction_uniformity(profile), 0.0)
    ...
    def test_perfectly_uniform_flow(self: "TestCalculateExtractionUniformity") -> None:
        ...
        self.assertEqual(self.analyzer.calculate_extraction_uniformity(profile), 1.0)
    ...
    def test_moderately_variable_flow(
        ...
        profile: FlowProfile = [
            ...
            FlowDataPoint(elapsed_time=float(i + 7), flow_rate=flow)
            for i, flow in enumerate(analysis_flows)
        ]
        # CoV = 0.3 / 1.5 = 0.2. Score = 1 - 0.2 = 0.8
        self.assertAlmostEqual(
            self.analyzer.calculate_extraction_uniformity(profile), 0.80
        )
    ...
    def test_highly_variable_flow(self: "TestCalculateExtractionUniformity") -> None:
        ...
        self.assertAlmostEqual(
            self.analyzer.calculate_extraction_uniformity(profile), 0.20
        )
    ...
    def test_uniformity_score_capped_at_zero(self):
        ...
        analysis_flows_very_high_cov = [0.1] * 9 + [4.1]  # Ensures CoV > 1
        profile: FlowProfile = [
            FlowDataPoint(elapsed_time=float(i), flow_rate=0.2 * i) for i in range(7)
        ] + [
            FlowDataPoint(elapsed_time=float(i + 7), flow_rate=flow)
            for i, flow in enumerate(analysis_flows_very_high_cov)
        ]
        self.assertAlmostEqual(
            self.analyzer.calculate_extraction_uniformity(profile), 0.0
        )
